...
HWY_INLINE SampleFunc ChooseSampleFunc(const RuntimeConfig& runtime_config) {
  // If user provided a custom SampleFunc, use it.
  if (runtime_config.sample_func) {
    return runtime_config.sample_func;
  }

  // Fast path for top-1 with no AcceptFunc filter.
  ...
  // General case: Softmax with top-k sampling and optional AcceptFunc filtering.
  return [&runtime_config](float* logits, size_t vocab_size) HWY_ATTR -> TokenAndProb {
    PROFILER_ZONE("Gen.Sample general");
    return FusedSoftmaxAndSampleTopK(
        logits, runtime_config.top_k, vocab_size,
        *runtime_config.gen, runtime_config.temperature,
